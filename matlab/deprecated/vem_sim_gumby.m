...
%     qdot=zeros(size(q));
    ...
    [w0, w0_I] = nurbs_blending_weights(parts, x0', 100, ...
        'Enable_Secondary_Rays', config.enable_secondary_rays);
...
%       [collide_verts, collide_faces] = triangulate_iges(collide_parts);
      [collide_verts, collide_faces] = readOBJ('./models/sphere.obj');
      collide_verts = .2 * collide_verts * (max(verts(:,1))-min(verts(:,1))) / (max(collide_verts(:,1))-min(collide_verts(:,1)));
...
%         config.collision_plane_z = min(verts(:,3)) - 20.0;
    ...
    for t=0:config.dt:40
        ...
        rhs = J' * (P*M*P'*J*qdot + f_internal + f_gravity + f_error + f_external + f_gumbo) + f_collision;
        qdot = lhs \ rhs;
