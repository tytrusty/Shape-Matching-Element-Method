function vem_simulate_nurbs_newcom(parts, varargin)
    ...
    % Shape Matrices
    L = compute_shape_matrices_newcom(x0, x0_coms, E, adjacent, ...
    ...
    % Per point center of masses
    x_coms_V = zeros(3,m);
    for i=1:m
        for j=1:numel(W_I{i})
            idx=W_I{i}(j);
            x_coms_V(:,i) = x_coms_V(:,i) + w(i,idx)*x_coms(:,idx);
        end
    end
%     vcom_plt = plot3(x_coms_V(1,:),x_coms_V(2,:),x_coms_V(3,:), ...
%                     '.','Color','m','MarkerSize',20);
%     hold on;
                             

    % Build Monomial bases for all quadrature points
    Y = monomial_basis_matrix(V, x_coms_V, config.order, k);
    
    stacked_coms = [];
    for i=1:n
        stacked_coms = [stacked_coms repmat(x_coms(:,i),1,numel(E{i}))];
    end
    Y0 = monomial_basis_matrix(x0, stacked_coms, config.order, k);
    ...
    dM_dX = monomial_basis_grad(V, x_coms_V, config.order);
    ...
    xplt=plot3(x(1,:),x(2,:),x(3,:),'.','Color','r');
...
%         for i=1:n
%             stacked_coms = [stacked_coms repmat(x_coms(:,i),1,numel(E{i}))];
% %             stacked_coms = [stacked_coms repmat(x0_coms(:,i),1,numel(E{i}))];
%         end
%         stacked_coms = stacked_coms(:);
% 
%         x_coms_bnd = zeros(3,size(x,2));
%         for i=1:size(x,2)
%             for j=1:numel(W0_I{i})
%                 idx=W0_I{i}(j);
% %                 x_coms_bnd(:,i) = x_coms_bnd(:,i) + w_x(i,idx)*x_coms(:,idx);
%                 x_coms_bnd(:,i) = x_coms_bnd(:,i) + w_x(i,idx)*x0_coms(:,idx);
%             end
%         end
%         stacked_coms = x_coms_bnd(:);
%         x_centered = x_centered- stacked_coms;
        %%
