...
function [ders]=dersbasisfuns(i,pl,u,u_knotl,k)

%     --------------variable declarations--------------------------------
% Input: i,pl,ml   %knot span, degree of curve, number of control points
%        u , u_knotl  %parameter value, vector of knots
%        k          % number of derivatives to compute
% Out:   ders(1,:)  % derivative matrix order 1
%        ders(2,:)  % derivative matrix order 2
nders=k;
left=zeros(1,pl+1);
right=zeros(1,pl+1);
ndu=zeros(pl+1,pl+1);
...
% Edit - Ty %
% this is what was needed to fix divide by 0 error below.
i=i+1;
% --------- %
...
saved = 0;
   for r = 0:j-1
       ...
       temp = ndu(r+1,j)/ndu(j+1,r+1);%%%%% error here (fix on ln37)
...
% Edit - Ty %
% I'd rather this only return derivatives.
ders = ders(2:end,:);
