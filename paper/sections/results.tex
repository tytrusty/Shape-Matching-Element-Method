...
\revise{We further perform basic \emph{quantitative} comparison against FEM by measuring the maximum deflection of the beam as we increase the number of patches along the beam. In \reffig{plot_beam_deflection}, the maximum beam deflection of the SEM beams is reported as a percentage of the FEM beam's maximum deflection. The parameters of the experiment match that of the previous qualitative test.} 
...
We use SEM with quadratic polynomials for this test and observe that our SEM simulation, made of \revise{22} independent NURBS patches, shows good agreement with FEM. Each subdivision of the NURBS beam enables more complicated kinematics, 
...
\revise{Additionally, we perform a twisting beam experiment on the 22 part beam model. Despite the relatively small number of NURBS patches, we see in \reffig{beam_twist} that the quadratic polynonmials permit complex deformation.}


\begin{figure}[h]
  \includegraphics[width=\columnwidth]{figures/beam_twist.pdf}
  \caption{\revise{With the left end of the beam held fixed, we show the result of rotating the right end of the 22 part beam up to 360 degrees.}}
  \label{fig:beam_twist}
\end{figure}


\revise{We perform a basic scalability test shown in \reffig{scalability} by iteratively adding more degrees of freedom to a model and measuring average runtime of a simulation step, and report the ratio of number of nonzeros in the stiffness matrix. We begin with a unit cube composed of 6 parts and on each iteration we extend the cube, adding 4 more parts. The raycasting quadrature is updated on each iteration so that a constant number of samples (45 in this case) lie in each cube. This represents a best case scenario in which sparsity is maximized as the number of degrees of freedom increases. As performance is tied to sparsity of the stiffness matrix, we see reasonable growth in the reported times, despite minimal effort on optimizing performance. This experiment was performed using a AMD Ryzen 2600 with 16GB of RAM}
...
\revise{Our soft enforcement of part connectivity at boundaries is a result of weighting function attributing equal weight values to equidistant parts. This equal weighting induces elastic forces that attempt to enforce continuity. In our examples this only introduces minor artifacts affecting the rendering, but this may be addressed using a separate rendering mesh or by using a representation with explicit connectivity.}
\revise{In \reffig{octopus} we demonstrate the shape-aware nature of our algorithm on a  stable Neo-Hookean ~\cite{SNHkim} simulation of an octopus, and observe independent motion for each limb.}

\begin{figure}[h]
  \includegraphics[width=\columnwidth]{figures/octopus.pdf}
  \caption{\revise{As a more extreme example of the shape-aware property of our algorithm, we simulate a falling octopus, and observe the 8 limbs moving independently. Self-collision is not accounted for here.}}
  \label{fig:octopus}
\end{figure}
