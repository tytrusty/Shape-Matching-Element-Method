...
\revise{We perform a basic scalability test shown in \reffig{scalability} by iteratively adding more degrees of freedom to a model and measuring average runtime of a simulation step, and report the ratio of number of nonzeros in the stiffness matrix. We begin with a unit cube composed of 6 parts and on each iteration we extend the cube, adding 4 more parts. The raycasting quadrature is updated on each
iteration so that a constant number of samples (45 in this case) lie in each cube. This experiment was performed using a AMD Ryzen 2600 with 16GB of RAM}

\begin{figure}[h]
  \includegraphics[width=\columnwidth]{figures/scalability.pdf}
  \caption{\revise{As a more extreme example of the shape-aware property of our algorithm, we simulate a falling octopus, and observe the 8 limbs moving independently.}}
