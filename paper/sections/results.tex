...
\begin{table*}[h]
  ...
  \caption{Performance and parameters for the Shape Matching Element Method on all examples. All wall-clock timings are reported in seconds, physical parameters are
...
\revise{We further perform a basic \emph{quantitative} comparison against FEM by measuring the deflection of the beam at equilibrium as we increase the number of patches along the beam. In \reffig{plot_beam_deflection}, the deflection of the SEM beams is reported as a percentage of the FEM beam's deflection.} 
...
\revise{Additionally, we perform a twisting beam experiment on a 22 part beam model. Despite the relatively small number of NURBS patches, we see in \reffig{beam_twist} that the quadratic polynomials permit complex deformation.}
...
We also show that our raycasting weight computation is able to create shape-aware output. \reffig{independence} and \revise{\reffig{octopus} show that manipulating
...
SEM also supports the simulation of objects made up of heterogeneous materials. By specifying different material properties inside nested 
...
%editing example
For downstream applications, the output of SEM simulation is itself editable in a NURBS modeling program (see \reffig{edit}). 
%This can be useful for visual effects pipelines, allowing artists to easily
% post-process animations using the same tools in which they were created.
This can allow artists to easily post-process animations using the same tools in which they were created.
\begin{figure}[h]
...
\begin{figure*}[htp]
  ...
  \includegraphics[width=\textwidth]{figures/starship_short.pdf}
