...
\begin{equation*}
    \begin{split}
        \tr_i  & = \sum_{\prt_k \in \bar{\refX}_j} \mathbbm{1}^T\wt_k \samp{\Jnurbs}_k\vc{q}_k \\
        \vc{c}_i^1 & = \left(\samp{\Pmat^1}^T\wt_i\samp{\Pmat^1}\right)^{-1}\samp{\Pmat^1}^T\wt_i\left(\samp{\Jnurbs}_i\vc{q}_i-\mathbbm{1}\tr_i\right)\\
        \vc{c}_i^2 & = \left(\samp{\Pmat^2}^T\wt_i\samp{\Pmat^2}\right)^{-1}\samp{\Pmat^2}^T\wt_i\left(\samp{\Jnurbs}_i\vc{q}_i-\samp{\Pmat^1}\vc{c}_i^1 - \mathbbm{1}\tr_i\right)
    \end{split},
    \label{eq:hfit} 
