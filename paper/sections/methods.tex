...
\begin{algorithm}[h]
	\begin{algorithmic}[1]
		...
		\State $\samp{\Jnurbs}, \wt \gets \text{SampleNURBS}(model)$  \Comment{Section \ref{sec:shapematching}}
...
\revise{
\begin{equation*}
    \phi_{ij}\left(u,v\right) = \frac{\omega_{i,j}\beta_{ik}(u)\beta_{jl}(v)}{\sum_{r=1}^{n}\sum_{s=1}^{m} \omega_{r,s} \beta_{rk}(u)\beta_{sl}(v)},
...
\begin{equation}
    ...
    \mathbf{p}(\refX-\bar{\refX})^T & \mathbf{0} & \mathbf{0} & 1 & 0 & 0\\
    \mathbf{0} & \mathbf{p}(\refX-\bar{\refX})^T & \mathbf{0} & 0 & 1 & 0\\
    \mathbf{0} & \mathbf{0} & \mathbf{p}(\refX-\bar{\refX})^T & 0 & 0 & 1\\
    \end{bmatrix}}_{\Pmatx}
    \Pcox,
    \label{eq:projection}
\end{equation} where $\mathbf{p}\left(\refX\right)^T$} is the polynomial basis vector \newline (e.g., $\begin{bmatrix} X & Y & Z & X\cdot Y & X\cdot Z & Y\cdot Z& X\cdot X & Y\cdot Y & Z\cdot Z \end{bmatrix}$ ),
...
Quadrature points are selected so that $\samp{\Jnurbs}$ is of sufficient rank required to represent all control points. We achieve this by sampling over each knot span defined by the  B-splines, and find it effective to use the order of the NURBS as the number of quadrature points sampled in each knot span. \revise{Positions and weights of the quadrature points in each span are chosen using Gauss-Lobatto quadrature.}
...
For a non-planar NURBS part, a sufficient number of quadrature points \revise{will make ~\refeq{shapematch} well-posed}, allowing us to write: $\Pcox^* = \Pi\vc{q}$ where
\begin{equation*}
...
When assembled for all parts,~\refeq{hfit}, yields a block upper triangular matrix $U$ and a sparse matrix $B$ ~(\reffig{multiparts}) which can be used to efficiently compute the projection 
operator for the entire object $\Pi = U^{-1}B$. 
...
\revise{Note that hierarchical shape matching is not guaranteed to find the optimal polynomial fit and potentially risks overfitting to low-order terms.
        However we find that it provides qualitatively reasonable results and avoids much of the dense matrix arithmetic and singular value decompositions 
        of alternative methods such as the Moore-Penrose inverse or QR factorization.} 
...
Automatic  weight computation is well-studied (see ~\cite{10.1145/2766952}, ~\cite{BBW:2011} and ~\cite{10.1145/2010324.1964968}), but typically requires solving 
...
\begin{figure}[h]
    \centering
    \includegraphics[width=3.33in]{figures/plot_distance_weight_both.pdf}
    \caption{Distance weight as a function of primary ray length $d_{\text{primary}}$ and total ray length $d_{\text{total}}$ with cutoff distance 50 (left). Here we use different colors to denote different parts (right).}
...
\label{eq:RELU}
\end{equation}
where $\alpha$ is a distance cutoff parameter, $d_{\text{primary}}$ is the primary ray length and $d_{\text{total}}$ is the total ray length. 
...
The price we pay is that our weights are not $C^0$ continuous but rather contain bounded jumps. 
...
While we constrain a part to be paired with only one deformation origin, an origin may be associated with any number of parts. 
...
\begin{figure}[h]
    ...
    \caption{Example structure for the network flow selection of deformation origins with a shape consisting of three parts. Red arrows indicate each part's origin association. \dave{make it clear edges of triangle are sep. parts, add capacities and costs}}
...
\begin{equation}
\begin{aligned}
\mathbf{f} =  \argmin_{\mathbf{f}} \quad & \mathbf{c}^T \mathbf{f}    \\
...
\revise{Note that this approach yields compactly supported weight functions due to the cut-off parameter~(Eq.~8). 
Depending on this value, we obtain a range of system matrix sparsity patterns -- from fully dense to block sparse~(\reffig{stiffness_sparsity}). 
Importantly, unlike other boundary-only approaches, our method is not guaranteed to produce dense system matrices.}
...
\begin{equation}
\int_{\Omega_R} g(X,Y,Z) d\Omega_R = \int_{\Gamma_R}\left(\int_{X_1}^{X_2} \revise{g(\alpha, Y, Z) d\alpha}\right)\mathbf{n} d\Gamma_R.
\end{equation}

where $\Gamma_R$ is the boundary of the bounding box with normals $\mathbf{n}$ and $X_1$ and $X_2$ represent the minimum and maximum values of the box along the $x$-axis. With this form, all we require to evaluate the outer integral is to integrate over the $y$-$z$ plane located at $X_1$. With this simple form, the normal term may be excluded as it always equals $1$ \dave{isn;t it a vector $(1,0,0)$ or something like that ? }in the case of an axis-aligned bounding box. Finally, raycasting is used to evaluate the line integral along the x-axis. From here, we find all the intersection intervals where the ray is inside $\Omega$, and select quadrature points uniformly along this interval. Thus, this transformation from the original volume integral into an integral over a $y$-$z$ plane plus axis-aligned rays enables us to integrate the volumes of our NURBS models in a completely meshless approach.

We make a minor modification to this procedure to handle common geometric pathologies encountered in NURBS modelling, such as overlapping surfaces, by performing 
...
\begin{algorithm}[h]
	\begin{algorithmic}[1]
		\Procedure{Simulate}{$model$}
     	 %\MM\ddot{\vc{q}} = \fo\left(\vc{q}\right) + \vc{f}_g + \fext,
     	\While {$simulating$}
     		\State{$\Pcox \gets \Pi\vc{q}$}
     		\ForAll{$i \gets 1$ to $|\st{X}|$}
     			\State{$\mathbf{F}_i \gets \frac{\partial \mathbf{F}}{\partial c} c \quad$ // deformation gradient}
				\State{$\frac{\partial V}{\partial c} \gets \frac{\partial V}{\partial c} +
				\frac{\partial \mathbf{F}}{\partial c}^T \frac{\partial \psi}{\partial \mathbf{F}}(\mathbf{F}_i) \cdot \text{volume}_i$}
  			...
  			\State{$\fo\left(\vc{q}\right) \gets \Pi^T \frac{\partial V}{\partial c} + \ME \vc{q}$}
  			\State{$\vc{f}_{\text{total}} \gets \fo\left(\vc{q}\right) + \vc{f}_g + \fext$}
  			\State{$\dot{\vc{q}} \gets \text{TimeIntegration}
  			(K, \vc{f}_{\text{total}}, \dot{\vc{q}}, \samp{\Jnurbs} , \MM, \ME)$}
  			\State{$\vc{q} \gets \vc{q} + h\dot{\vc{q}} \quad$ // $h$ is timestep size}
     	\EndWhile
	\EndProcedure
\end{algorithmic}
...
\end{equation} where $\shapef\in\real^{3\times n} = \begin{bmatrix}P_1\left(\refX\right) \dots P_m\left(\refX\right)\end{bmatrix}$, \revise{$n$ is of size $3(km + |\XBs|)$ with $k$ being the number of terms in each polynomial, $m$ is the number of parts, and $|\XBs|$ is the number of deformation origins. \ty{previously said:$m$ is the number of 
...
\begin{equation}
    T = \frac{1}{2}\dot{\vc{q}}^T\underbrace{\Pi^T\int_{\Omega} \revise{\rho}\bldmat\left(\refX\right)^T\shapef\left(\refX\right)^T\shapef\left(\refX\right)\bldmat\left(\refX\right)d\Omega\Pi}_{\MM}\dot{\vc{q}}.
\end{equation}
