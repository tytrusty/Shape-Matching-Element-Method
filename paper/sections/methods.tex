...
\dave{explain NURBS but not D-NURBS because that's Terzopolous's specific algorithm just explain the NURBS parameterization}
...
\dave{Shape Matching to a single NURBS part}
...
\dave{here explain sampling on NURBS surface to get material points}
...
\dave{weights are missing, just matching to points leaves us vulnerable to biasing towards clusters of points}
...
\dave{need to explain that system can be degenerate fix is to select a constant term}
\begin{equation}
...
\dave{no b just use J q, here's a good place to explain sampling the surface to build big J.}
\begin{equation}
...
\dave{Shape Matching is VEM brief explanation here. Point of departure, one polynmoial isn't going to cut it for a whole object.}
\subsection{Shape Matching Element Method}
\dave{Multiple Parts}
\dave{each polynomial/NURBS tuple is a shape matching element}
\dave{Shape Matching Element assembles global kinematics using blendeded formultion}
