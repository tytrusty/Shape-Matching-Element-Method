...
\begin{figure}[h]
    ...
    \caption{A simple, four part model with two deformation origins and the corresponding matrix equation for the quadratic coefficients. \ty{these $T^1$ and $T^2$ blocks in the U matrix need to be swapped I think} }
...
Finally, $\wt_i$ is the diagonal integration weight matrix and $\mathbbm{1}\in\real^{3n\times3}$ is a matrix of stacked $3\times3$ identity matrices -- one for each quadrature point. \ty{it looks like we are missing the divisor for t? This looks like it's computing the sum and $\mathbbm{1}$ should be defined as $1/N$ where $N$ is the total number of points in all associated parts}
...
Further, we require that each deformation origin be associated with a minimum of two parts, which avoids the case of an origin laying on the surface of a part. One exception to this rule is periodic NURBS surfaces, where we allow for a deformation origin associated with only this part. Finally, to avoid unduly coupling the parts and limiting the expressivity of the simulation, we seek a sparse association of origins which mirrors the pointwise sparsity of our blending weights.
...
To assign deformation origins to parts we use an integer network flow approach~\dave{cite}~(\reffig{network_flow}). 
...
We solve this problem with the following integer constrained linear program
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
where $\mathbf{f}$ and $\mathbf{c}$ are the edge flow and cost values, respectively, assembled into vectors, and $A_{\text{cons}}$ forms the set of constraints satisfying the flow conservation constraint. \ty{we use the default in intlinprog, which should be dual-simplex} We choose $\XBs$ to be all the deformation origin candidates in $A$ that have an outgoing flow of at least $1$. 
...
The advantage of having a unified kinematic description~(\refeq{kinematics}) is that we can directly apply Lagrangian Mechanics~\dave{cite Lanczos again} to arrive at 
