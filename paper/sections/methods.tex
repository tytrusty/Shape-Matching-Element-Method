...
From the set of blending weights we seek to construct a set of candidate deformation origins and then produce a set of deformation origins, $\XBs$, adhering to the outlined requirements. These requirements admit a graph structure ~(\reffig{multiparts}), which we exploit by forming a simple network flow problem that produces a solution satisfying the requirements. The relationships between parts and deformation origins can be realized as a bipartite min-cost-flow problem.

Consider a directed bipartite graph $G=(V = A \cup B,E)$ where each node in $A$ corresponds to a deformation origin candidate and each node in $B$ represents one part. $E$ is the set of edges connecting nodes in $A$ to $B$ where each origin has an each to every part with which it attempts to associate. We augment $G$ to form $G'=(V' = A \cup B \cup \{s,t\}, E')$ where $V'$  is the new set of nodes now containing the source, $s$, and sink, $t$, nodes. $E'$ is the augmented set of nodes connecting $s$ to each candidate in $A$ and connecting each part in $B$ to the sink $t$. Each edge $(i,j) \in E'$ has an associated flow $f_{ij}$ and capacity constraint $u_{ij}$, and each edge in $E'$ also has a cost $c_{ij}$. The principal constraint for this network flow problem is the conservation constraint, meaning that the net flow for all nodes in $V$ is zero.

First, we constrain all flow values to be integral and place a capacity of $1$ on the edges from $B$ to $t$. This has the effect of enforcing that any node in $B$ receives flow from a single edge, and as a consequence each part may only be associated with a single deformation origin. 
Flow between a part and origin is constrained to binary values and a flow value on edge between a part and origin candidate indicate that the part is paired with this deformation origin. We organize the problem so that the source node is connected to each candidate origin node, and the sink node is connected with each part. We place a capacity constraint of $1$ on the sink's edge, which enforces the constraint that each part is paired with one origin.
