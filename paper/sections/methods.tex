...
For a non-planar NURBS part, a sufficient number of quadrature points will make ~\refeq{shapematch} will be well-posed, allowing us to write: $\Pcox^* = \Pi\vc{q}$ where
\begin{equation*}
...
\begin{equation}
\int_{\Omega_R} g(X,Y,Z) d\Omega_R = \int_{\Gamma_R}\left(\int_{X_1}^{X_2} g(\alpha, Y, Z) d\alpha)\right)\mathbf{n} d\Gamma_R.
\end{equation}
...
\end{equation} where $\shapef\in\real^{3\times n} = \begin{bmatrix}P_1\left(\refX\right) \dots P_m\left(\refX\right)\end{bmatrix}$, $m$ is the number of 
