...
\begin{algorithm}[h]
	\begin{algorithmic}[1]
		...
		\State $\Pi \gets \text{ShapeMatching}(\vc{q}, \samp{\Jnurbs}, \wt, \XBs)$ \Comment{Section \ref{sec:shapematching}}
		\Statex
...
Let $\vc{q^0}$ be the initial control points of the part, provided by the modeller, and $\vc{q}$ be a vector of modified control point values.
...
increase the number of polynomials in the shape matching. Next we outline our meshless Shape Matching Element Method which works directly on the NURBS
geometry itself.
...
\section{Shape Matching Element Method}
 ...
 \begin{equation}
    ...
    \label{eq:kinematics}
 \end{equation} where $\bldxi$, $\Pmatx_i$, $\Pi_i$ and $\vc{q}_i$ are the blending weights, polynomial basis, projection operator and control points for the $i^{th}$
 part respectively.  
