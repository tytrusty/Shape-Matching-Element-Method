...
Specifying weights manually would be antithetical to our goal of providing a seamless translation from surface representation to simulation.
Automatic  weight computation is a well-studied topic (for examples ~\citet{10.1145/2766952,BBW:2011} and ~\citet{10.1145/2010324.1964968}) but typically requires solving an optimization problem
that itself relies on a volumetric discretization. 
This can be avoided by using stochastic methods~\cite{Sawhney:2020:MCG} but do not yet support the jump boundary conditions we require for our problem. 
...
These properties ensure that our simulation can properly represent motions up to the order of the polynomial basis and that the solution stored at each part is the actual deformed position.
...
While a partition of unity and sparsity are straightforward to enforce, the overlapping and intersecting parts~(\reffig{badmodels}) often encountered in non-engineering models lead
