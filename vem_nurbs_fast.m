function vem_nurbs_fast
    ...
    function nurbs = plot_srf(nurbs)
            ...
            for ii = 1:numel(nurbs)
                ...
                plt = surf(squeeze(xi(1,:,:)),squeeze(xi(2,:,:)),squeeze(xi(3,:,:)),'FaceAlpha',0.9,'EdgeColor','none','FaceColor',cm(ii,:));
                hold on;
                nurbs{ii}.plt=plt;
            end
    end
...
%     part=nurbs_from_iges('rocket.iges',7,0);
    part=nurbs_from_iges('rounded_cube.iges',7,0);
    part=plot_srf(part);
...
%         hold on;
    ...
    for i=1:numel(E)
        ...
        min_dist =min(diff',[],2);
        distances = min(diff',[],2) < threshold;
        distances(E{i}) = 0;
        verts_to_weld = find(distances);
        %h1=plot3(x0(1,verts_to_weld),x0(2,verts_to_weld),x0(3,verts_to_weld),'.','Color','r','MarkerSize',30);
        S_w{i} = sparse(zeros(numel(x0), numel(verts_to_weld)*3));
        S_I{i} = verts_to_weld;
        for j=1:numel(verts_to_weld)
            idx = verts_to_weld(j);
...
%     hold on;
    ...
    for t=0:dt:30
        ...
        if save_output
            fn=sprintf('output_png\\pin_bottom_fail_%03d.png',ii)
            saveas(fig,fn);
        end
        ii=ii+1;
    end
